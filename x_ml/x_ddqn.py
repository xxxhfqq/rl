...
"""
x_ml.x_ddqn
只改了一个地方 就是update函数中,选择最佳动作时,仍然使用q_net, 计算价值时才使用target_net
"""
import numpy as np
import torch
...
class DDQN():
    ...
    def update(self):
        ...
        with torch.no_grad():
            self.q_net.eval()
            best_action = self.q_net(next_state).argmax(1).unsqueeze(1) ### DDQN修改的地方
        ...
        with tqdm(total=self.num_episodes) as pbar:
            for i in range(self.num_episodes):
                ...
                if (i + 1) % self.eval_step == 0:
                    self.eval(pbar)
                pbar.update()
                pbar.set_postfix(return_reward=total_reward, max_eval_reward=self.max_reward, epsilon=self.epsilon)
